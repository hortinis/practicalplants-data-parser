...
Common-name alias pages are emitted under `alias/`. Family, usage (including plant-part and animal-forage lists), category and catalogue list pages are emitted under `collection/`; genus lists remain under `index/` for backward compatibility. Collections include `completeness: "populated"` or `"empty"` so an empty generated list is not confused with a parser failure. Their `memberSource` distinguishes archive lists, inverse use/category/ecology recovery, and explicitly derived function intersections. Plant pages preserve ecology function links with source locations. Category collections additionally preserve subcategories, the archive's reported total member count, and whether the archived page contains the complete member list.
