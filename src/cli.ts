...
import { parseConcept, parseIndex, parseUnknown } from './parser/non-plant.js';
...
for (const candidate of scan.pages) {
  try {
    const html = await readFile(candidate.absolutePath, 'utf8'); const $ = load(html); const type = classifyPage($); const common = [candidate.pageId,candidate.sourcePath,'Practical Plants recovered archive',scan.commit,pageIds] as const;
    const page = type === 'plant' ? parsePlant($, ...common) : type === 'concept' ? parseConcept($, ...common) : type === 'index' ? parseIndex($, ...common) : parseUnknown($, ...common);
