...
await writeOutput(output,pages,errors,{ parserVersion:'0.13.0', sourceRepository:'Practical Plants recovered archive', sourceCommit:scan.commit, schemaVersion:'0.13.0', pageCounts:counts, collectionCounts, collectionRecovery, categoryRecovery, ...(imageRecovery ? { imageRecovery } : {}), pageCount:pages.length, errorCount:errors.length });
